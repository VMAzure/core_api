...
def add_service(service: ServiceCreate, Authorize: AuthJWT = Depends(), db: Session = Depends(get_db)):
    ...
    logger.info("✅ Inizio creazione servizio")
    
    try:
        new_service = Services(name=service.name, description=service.description, price=service.price)
        db.add(new_service)
        logger.info("✅ Servizio aggiunto al database, prima del commit")
        db.commit()
        db.refresh(new_service)
        logger.info(f"✅ Servizio creato con successo: ID {new_service.id}")
