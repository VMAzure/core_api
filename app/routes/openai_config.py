...
def _mask_url(u: str) -> str:
    try:
        ...
        # Mostra host+path, nascondi query firmata
...
async def gemini_auto_scenario(
    ...
    _logger.info("DEBUG img1 type=%s val=%r", type(img1), img1)
    _logger.info("DEBUG img2 type=%s val=%r", type(img2), img2)
    ...
    _logger.info("AUTO-SCENARIO start user=%s id_auto=%s img1=%s img2=%s",
                 user_email, str(payload.id_auto), _mask_url(img1), _mask_url(img2))
    ...
    rec_clean = UsatoLeonardo(
        ...
        model_id="gemini-2.5-flash-image-preview",
        aspect_ratio="16:9",
        credit_cost=0.0,             # nessun addebito per step tecnico
        user_id=user.id,
        subject_url=img1,
        is_deleted=True              # nascosto al FE
    )
    ...
    try:
        ...
        img_bytes = img_bytes_list[0]  # usa la prima immagine
        _, signed_urlA = _sb_upload_and_sign(pathA, img_bytes, "image/png")

        _logger.info("STEP A generated bytes=%d time=%.2fs", len(img_bytes or b""), time.perf_counter() - tA)
        ...
        rec_clean.status = "failed"; rec_clean.error_message = str(he.detail)
        db.commit()
        _logger.error("STEP A HTTPException: %s", he.detail)
        raise HTTPException(500, f"Errore generazione step A (pulizia): {he.detail}")
    except Exception as e:
    ...
    _logger.info("STEP B queued rec_final_id=%s subj=%s back=%s",
                 str(rec_final.id), _mask_url(rec_clean.public_url), _mask_url(img2))

    try:
        tB = time.perf_counter()
        _logger.info("DEBUG STEP B subj type=%s val=%r back type=%s val=%r",
             type(rec_clean.public_url), rec_clean.public_url,
             type(img2), img2)
        ...
        _logger.info("STEP B generated bytes=%d time=%.2fs", len(img_bytesB or b""), time.perf_counter() - tB)
        ...
        rec_final.status = "failed"; rec_final.error_message = str(he.detail)
        db.commit()
        _logger.error("STEP B HTTPException: %s", he.detail)
        raise HTTPException(500, f"Errore generazione step B (composizione): {he.detail}")
    except Exception as e:
